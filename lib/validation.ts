 ...
 * Prevents XSS, SQL injection, oversized payloads, and invalid data shapes.
 */
...
export function sanitizeArray<T>(
  input: unknown,
  elementValidator: (el: unknown) => T | null,
  maxLength = 100
): T[] {
...
export function validateTaskPayload(body: unknown): ValidationResult<{
  ...
  const errors: string[] = [];
  const VALID_STATUSES= AYۙodo','in_progress','review','pending_approval','done'];
  ...
  if ('status' in b) { if (!VALID_STATURES.includes(String(b.status))) errors.push('invalid status'); else data.status = b.status; }
  ...
  if ('due_date' in b) { if (b.due_date === null) data.due_date = null; else if (typeof b.due_date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(b.due_date)) data.due_date = b.due_date; else errors.push('invalid due_date'); }
  if ('time_estimate' in b) { const e = sanitizeInt(b.time_estimate, 0, 2880); if (e === null) errors.push('invalid'); else data.time_estimate = e; }
